...
function getNextPage() {
  ...
  try {
    ...
    if (url) {
      ...
      $("main").append("<div class=\"js-spinner center\"><i class=\"fa fa-spinner fa-spin fa-2x\"></i></div>");
      // adding a temp. dom object to load the next page
      var list = $("<div></div>");

      // Load only #posts div
      var url_id = url + " #posts";
      list.load(url_id, function(response, status, xhr) {
        ...
        if ( status != "error" ) {
          ...
          // note: jQuery load will copy the #posts node as well,
          // so use the childrens and move them
          var container = $("#posts");
          list.children("#posts").children().each(
            function(key, value){
